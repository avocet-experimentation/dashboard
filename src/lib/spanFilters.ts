...
const spansForExperiment = (spans, experiment: Experiment, group: ExperimentGroup, treatment: Treatment) => {
  ...
  return spans.filter(span => {
    ...
    Object.keys(span.spanattributes).forEach(key => {
      ...
      if (span.spanattributes[key] === targetHash) { match = true }
    });
...
export { spansWithFlags, spansWithFlagName, spansForExperiment, getPropertyValueArray }
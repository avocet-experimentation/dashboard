    ...
    <>
      <Field label={`Treatments (${Object.keys(definedTreatments).length})`}>
            ...
            ([id, treatment], treatmentIdx) => (
              ...
              >
                ...
                <Table.Root stickyHeader interactive>
                  ...
                  <Table.Body>
                    {definedTreatments[id].flagStates.map(
                      (treatmentFeatureObj, featureIdx) => (
                        <Table.Row key={id + String(featureIdx)}>
                          ...
                          <Table.Cell>
                            <Controller
                              ...
                              control={control}
                              render={({ field }) => {
                                const selectedFeatureId = definedTreatments[id].flagStates[featureIdx]
                                  .id;
                                if (selectedFeatureId) {
                                  ...
                                  if (selectedFeature.type === 'boolean') {
                                    return (
                                      <Switch
                                        id={treatmentFeatureObj.id}
                                        key={treatmentFeatureObj.id}
                                        name={field.name}
                                        checked={!!field.value}
        ...
        <Button
          ...
          onClick={() => {
            // Object.values(featureFieldArrayHelpers).forEach(({ append }) => {
            //   append({});
            // });
            Object.keys(definedTreatments).forEach((id) => {
              const currentFlagStates = [
                ...definedTreatments[id].flagStates,
                {},
              ];
              setValue(`definedTreatments.${id}.flagStates`, currentFlagStates);
            });
          }}
        >
          <CirclePlus />
                          ...
                          <Input
                            ...
                            {...register(`groups.${index}.proportion`, {
                              ...
                              validate: {
                                sumToOne: () =>
                                  groupValues.reduce(
                                    (acc: number, { proportion }) =>
                                      acc + proportion,
                                    0,
                                  ) === 1 || 'Group proportions must sum to 1.',
                              },
                            })}
                          />
                        </Field>
            ...
            >
              <Button
                ...
                onClick={() => {
                  ...
                  const firstTreatmentId = Object.keys(definedTreatments)[0];
                  const currentFlagStates = definedTreatments[firstTreatmentId].flagStates;
                  const templateFlagStates = currentFlagStates.map(
                    ({ id, value }) => ({ id, value: undefined }),
                  );
                  newTreatment.flagStates = templateFlagStates;
